...
export default class extends Base {
    async detailAction(){
        ...
        let list    = await this.model('coursemind').field([
                ...
                'expanded',
                'url'
            ]).where({cid:param.id ,cuid:param.uid}).order('nid').select();
    ...
    async saveAction(){
        ...
        let param = this.post() , cid   = param.catalogId ;
        if(!cid) return this.fail('参数错误！');
        let list  = JSON.parse(param.list) ;

        let newList = [];
        list.map((k) =>{
            newList.push({nid:k.id ,pnid:(k.parentid||'') ,title:k.topic ,direction:(k.direction||'right') ,expanded:(k.expanded||'1') ,cuid:cuid ,cid:cid ,url:(k.url||'')});
        });
        // 删除所有，之后再增加
        let model = this.model('coursemind');
        await model.delete({where: {cuid: cuid, cid:cid}});
