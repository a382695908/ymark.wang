					...
					"items": {
						...
						"editContent": {
							...
							"action": function(data) {
									...
									load_article = function(callback) {
										...
										if (objContent.getSaveStatus() == 1) {
											layer.confirm('文章还没有被保存！要不要保存文章后再加载？', {
												...
												objContent.loadArticle(cid, callback);
											});
											return;
										} else {
											objContent.loadArticle(cid, callback);
								...
								// 先加载文章内容
								if (extFun.getEditType() == '1') {
									load_article(load_mind);
								}
								// 先加载MIND
								else {
									load_mind(load_article);
								}
							}
						},
						"addNode": {
...
function courseMind() {
	...
	$btnHelp.click(function() {
		if (layindex_help > 0) {
			...
			layindex_help = layer.tips(htmlHelp, $btnHelp, {
				tips: [3, '#78BA32'],
				time: 0
			});
		}
	...
	$btnNewCN.click(function() {
		...
		if (!selected_node) {
			$.error('please select a node first.');
		}
	...
	return {
		...
		quickSave: function(callback) {
			...
			var data = objDiagram.get_data('node_array').data,
				fun_save = function() {
					$.query('/mcourse/cm/' + quid, {
						list: JSON.stringify(data),
						catalogId: objContent.getId()
					}, callback, 'put');
				}
			if (data.length <= 1) {
				layer.confirm('确认要删除节点吗？', {
					btn: ['一定要删除', '还是取消吧']
				}, function(i) {
					layer.close(i);
					fun_save();
