;(function(window,jQuery){
    jQuery.extend( {
    	urlVal : function(name) {
    		...
    		if (!r) {return window.unescape(r[2]);}
    		return "";
    	},
    	getNow : function(){
    		var now = new Date(),
        ...
        getHeight : function(){
            ...
            if(window.navigator.userAgent.indexOf("Firefox")>0){_t = window.document.documentElement; _height = _t.scrollHeight; }
              ...
              if(window.addEventListener) {
                ...
                //  alert('IE8 - 2')
                //}
                _t=window.document.body; _height=_t.scrollHeight;

              } //其他浏览器:谷歌 ，360[7.1]
              else if(window.attachEvent) {_t=jQuery(window); _height=_t.height();} //> ? =IE8 [升级到11的IE8]
