...
_e(function (E, $) {
    ...
    Txt.fn.contentEmptyHandle = function () {
        ...
        $txt.on('keyup', function (e) {
            ...
            // 当元素为最后一个li时，直接删除ul,ol
            var $last       = $(editor.currentRange().startContainer) ,
                last_tag    = $last.get(0).tagName ,
            ...
            if(last_tag == 'LI' || last_tag == 'li'){
                var $sibling = $last.siblings('li');
                // 判断是否为最后一个li
                if($sibling.length == 0){
                    $last.parent('ul,ol').remove();
                    insertEmpty();
                    return;
                }
            }
            if($last.html() == '<br>'){ //当只有br的时候，删除当前元素
                $last.remove();
                insertEmpty();
            }
    ...
    Txt.fn.bindTabEvent = function () {
        ...
        $txt.on('keydown', function (e) {
            ...
            // 如果浏览器支持 insertHtml 则插入4个空格。如果不支持，就不管了
            var support = editor.queryCommandSupported('insertHtml');
            if (!support) {
                return;
            }
            // 当Tab的元素等于LI，且内容为空时，增加子ul/ol
            // 存在两种情况：
            //  1.当先单击列表，然后添加内容时，生成的HTML = <li>11</li>
            //  2.当选中一段文字，再单击列表时，生成的HTML = <li><span>11</span></li>
            var $last       = $(editor.currentRange().startContainer) ,
                last_tag    = $last.get(0).tagName ;
            if(last_tag == 'LI' || last_tag == 'li'){
                if($last.html() == '<br>'){ //等于空
                    $last.after('<ul><li></li></ul>');
                    $last.remove();
                    editor.command(e, 'insertHtml', '');
                }
                return;
            }
            var $pli        = $last.parent('span').parent('li') ;
            if($pli.length > 0){
                last_tag_p  = $pli.get(0).tagName ;
                if(last_tag_p == 'LI' || last_tag_p == 'li'){
                    console.log('h');
                    return;
                }
            }
    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        $txt.on('paste', function (e) {
            var $current = $(editor.currentRange().startContainer);
            ...
            if (data && data.getData) {
                ...
                if($current.parents('li').length > 0){  //则粘贴纯文本
                    resultHtml = data.getData('text/plain');    // 得不到html，试图获取text
                }else{  // 粘贴HTML
                    // 获取粘贴过来的html
                    pasteHtml = data.getData('text/html');
                    if (pasteHtml) {
                        // 过滤掉不需要的属性
                        resultHtml = pasteHtml.replace(editor.config.pasteRegex, '')
                    }else{
                        resultHtml = data.getData('text/plain');    // 得不到html，试图获取text
                    }
                }
